...
impl Router {
    ...
    fn router() -> matchit::Router<u32> {
        ...
        router.insert("/todos", 14u32).unwrap();
        router.insert("/todos/{id}", 15u32).unwrap();
        router.insert("/todos/{id}/edit", 16u32).unwrap();
        router.insert("/todos/{id}/toggle", 17u32).unwrap();
        router
    }
    pub async fn route(
        &self,
...
pub mod route_1 {
    ...
    async fn handler(
        ...
        let v5 = app::routes::pages::labels_page(&v4, v2).await;
        let v6 = match v5 {
            Ok(ok) => ok,
            Err(v6) => {
                return {
                    let v7 = app::routes::pages::LabelsPageError::into_response(&v6);
...
pub mod route_10 {
    ...
    async fn handler(
        ...
        let v8 = app::routes::pages::history_page(v5, &v7, v3).await;
        let v9 = match v8 {
            Ok(ok) => ok,
            Err(v9) => {
                return {
                    let v10 = app::routes::pages::HistoryPageError::into_response(&v9);
                    let v11 = pavex::Error::new(v9);
                    app::telemetry::error_logger(&v11, v1).await;
                    <pavex::Response as pavex::IntoResponse>::into_response(v10)
                };
...
pub mod route_13 {
    ...
    async fn handler(
        ...
        let v5 = app::routes::pages::todos_page(&v4, v2).await;
        let v6 = match v5 {
            Ok(ok) => ok,
            Err(v6) => {
                return {
